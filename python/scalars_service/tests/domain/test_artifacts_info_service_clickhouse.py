"""Unit tests for ``ArtifactsInfoService`` ClickHouse table lifecycle helpers."""
...
def _svc(client: AsyncMock) -> ArtifactsInfoService:
    return ArtifactsInfoService(client, last_logged_service=None)
...
async def test_drop_clickhouse_table_issues_drop(project_id) -> None:
    ...
    expected = SCALARS_DB_UTILS.safe_artifacts_info_table_name(project_id)
    client.command.assert_awaited_once()
...
async def test_get_clickhouse_table_usage_stats_when_missing(project_id) -> None:
    ...
    assert stats.table == SCALARS_DB_UTILS.safe_artifacts_info_table_name(project_id)
...
async def test_get_clickhouse_table_usage_stats_when_present(project_id) -> None:
    ...
    client.query = AsyncMock(
        side_effect=[
            ...
            MagicMock(result_rows=[("c1",), ("c2",), ("c3",)]),
            MagicMock(result_rows=[[40]]),
        ]
    )
    svc = _svc(client)
...
async def test_drop_managed_table_by_name_rejects_bad_prefix() -> None:
    ...
    with pytest.raises(ValueError, match="managed"):
        await svc.drop_managed_table_by_name("scalars_x")
...
async def test_drop_managed_table_by_name_issues_drop() -> None:
    client = AsyncMock()
    svc = _svc(client)
    await svc.drop_managed_table_by_name("artifacts_info_abcd0000")
    client.command.assert_awaited_once()
    assert "artifacts_info_abcd0000" in client.command.await_args.args[0]
