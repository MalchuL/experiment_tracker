"""
Tests for the experiments controller (API endpoints).
"""
...
def create_test_app() -> FastAPI:
    """Create a test FastAPI app with required routers."""
    app = FastAPI()
...
def _create_team(client: TestClient, name: str = "Experiment Team") -> str:
    response = client.post(
        "/api/v1/teams",
        json={"name": name, "description": "Team for experiment tests"},
    )
...
class TestExperimentControllerCreate:
    async def test_create_experiment_as_member(
    ...
    async def test_create_experiment_denied_for_viewer(
        self, auth_client, test_user: User, test_user_2: User
    ):
        owner_client = auth_client(test_user)
        team_id = _create_team(owner_client)
...
class TestExperimentControllerAccess:
    async def test_get_experiment_accessible_for_viewer(
        self, auth_client, test_user: User, test_user_2: User
    ):
        owner_client = auth_client(test_user)
        team_id = _create_team(owner_client)
        ...
        viewer_client = auth_client(test_user_2)
        response = viewer_client.get(f"/api/v1/experiments/{experiment['id']}")

        assert response.status_code == 200
        assert response.json()["id"] == experiment["id"]

    async def test_update_experiment_denied_for_viewer(
        self, auth_client, test_user: User, test_user_2: User
    ):
        owner_client = auth_client(test_user)
        team_id = _create_team(owner_client)
    ...
    async def test_delete_experiment_denied_for_viewer(
        self, auth_client, test_user: User, test_user_2: User
    ):
        owner_client = auth_client(test_user)
        team_id = _create_team(owner_client)
        ...
        viewer_client = auth_client(test_user_2)
        response = viewer_client.delete(f"/api/v1/experiments/{experiment['id']}")
