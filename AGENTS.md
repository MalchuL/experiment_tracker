...
For a **single public UI origin** without editing a root `.env`, run **`./scripts/docker-up-public.sh`** for `docker-compose.yml` or **`./scripts/docker-up-dev.sh`** for `docker-compose.dev.yml` from the repository root (see root **`DOCKER.md`** → *Custom URL or domain* → *One command without a `.env` file*).

For local development against the backend, set the web env so the UI targets the browser-reachable API (for example `PUBLIC_API_BASE_URL=http://127.0.0.1:8000`). In Docker Compose, the **`web`** service reads `PUBLIC_API_BASE_URL` at runtime and injects it into the frontend; it also sets **`SERVER_API_BASE_URL=http://backend:8000`** so server-side Route Handlers proxy to the API over the Compose network. For a **custom public URL** (real hostname or HTTPS, not only localhost), set **`PUBLIC_API_BASE_URL`** to that API origin, set **`ALLOWED_ORIGINS`** / **`OBJECT_STORAGE_ALLOWED_ORIGINS`** to your UI origin(s), and recreate **`web`**, **`backend`**, and **`object-storage`** — see the root **`DOCKER.md`** section **Custom URL or domain**.
