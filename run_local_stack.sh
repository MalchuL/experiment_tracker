...
launch_terminal \
  ...
  "cd python/object_storage && uv run uvicorn object_storage.main:app --reload --port 8002 --log-level debug"
sleep 2
...
launch_terminal \
  "local-run: backend" \
  "cd python/backend && uv run uvicorn api.main:app --reload --port 8000 --log-level debug"
sleep 2
